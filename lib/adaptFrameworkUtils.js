...
const grunthelpers = require('grunthelpers');
...
class AdaptFrameworkUtils {
  ...
  */
  ...
  static async loadFramework(dir) {
    const h = new grunthelpers({ rootPath: dir });
    h.load();

    // const schema = await App.instance.waitForModule('jsonschema');
    // console.log(Object.keys(schema.schemaPaths).map(k => `${k}: [${schema.schemaExtensions[k] || ''}]`));
    // console.log(await schema.getSchema('multipleChoiceQuestion'));

    // console.log('==========>> getData', data.languages[0].files.map(f => f.data.length ? f.data.map(getType) : getType(f.data)));

    // console.log('==========>> getData', h.getData());
    console.log('==========>> getSchemas', h.getSchemas());

    /*
    console.log('==========>> getData', data.languages[0].files.map(f => {
      return f.fileItems.map(fi => {
        if(fi.file.data.length) {
          return fi.file.data.map(getType);
        }
        return getType(fi.file.data);
      });
    }));
    */
    // console.log('==========>> getPlugins', h.getPlugins());
  }
  /**
